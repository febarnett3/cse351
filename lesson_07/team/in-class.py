...
class Board():
    ...
    def __init__(self):
        ...
        for d in self.directions:
            self.processes.append(mp.Process(target = self.check_location, args = (self.que,self.results,d,)))
    ...
    def check_location(self, que, output, d):
        while True:
            value = que.get()
            if value is None:
                return
    ...
    def find_word(self, word):
        ...
        for row in range(self.size):
            for col in range(self.size):
                ...
                processes = []
                for d in range(0, 8):
                    p = mp.Process(target= self._word_at_this_location, args = (row, col, d, word))
                    processes.append(p)
                    p.start()
                success = False
                for p in processes:
